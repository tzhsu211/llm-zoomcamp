...
def main():
    ...
    # Initialize session state only if 'conversation_id' does not exist
    ...
    # Display current conversation ID for debugging
    print_log(f"Current conversation ID-2: {st.session_state.conversation_id}")
    ...
    print_log(f"Current conversation ID-3: {st.session_state.conversation_id}")
    ...
    if st.button("Ask"):
        ...
        with st.spinner("Processing..."):
            ...
            print_log(f"Current conversation ID-4: {st.session_state.conversation_id}")
            print_log(f"Saving conversation to database with ID: {st.session_state.conversation_id}")
            save_conversation(
    ...
    with col1:
        if st.button("+1"):
            print_log(f"Current conversation ID-5: {st.session_state.conversation_id}")
    ...
    with col2:
        if st.button("-1"):
            print_log(f"Current conversation ID-6: {st.session_state.conversation_id}")
            st.session_state.count -= 1
    ...
    print_log(f"Current conversation ID-7: {st.session_state.conversation_id}")
